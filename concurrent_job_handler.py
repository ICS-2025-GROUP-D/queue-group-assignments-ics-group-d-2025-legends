...
class ConcurrentJobHandler:
    """
    Module 4: Handles concurrent job submissions for the print queue system.
    ...
    def handle_simultaneous_submissions(self, print_queue, jobs: List[Tuple[str, int]]) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=min(self.max_threads, len(jobs))) as executor:
            # Submit all jobs concurrently
            future_to_job = {
                executor.submit(self._submit_single_job, print_queue, user_id, priority, idx): (user_id, priority, idx)
    ...
    def format_submission_results(self, result: Dict[str, Any], job_count: int) -> str:
        """
        ...
        for r in result['results']:
            ...
            job_info = f"Job#{r['job_id']}" if r['job_id'] else "No Job ID"
            output_lines.append(
                f"  {status} {r['user_id']} (pri:{r['priority']}) -> {job_info} | {r['message']}"
            )
